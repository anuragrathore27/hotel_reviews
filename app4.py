...
def Prediction(corpus):
    ...
    # TFIDF -Pickel file
    loaded_TFIDF = load(open('vect.sav', 'rb'))
    ...
    #model pickle file
    loaded_model= load(open('lg.sav','rb'))
...
def keywords(corpus):
    ...
    attributes=extract_topn_from_vector(feature_names,sorted_items,5)
...
def main():
    ...
    <h1 style ="color:black;text-align:center;">Hotel Review Classification.</h1> 
    </div> 
    """
    ...
    if st.button("Predict"):
        ...
        st.success('The Review of the hotel is {}'.format(predict))
        
    #if st.button("IMP Attributes"):
        st.subheader("Important Attributes in Review")
